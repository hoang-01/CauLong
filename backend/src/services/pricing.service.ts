...
export class PricingService {
    static async calculateTotalPrice(
        ...
        const configs = facilityConfigs.filter(c => c.court_type === courtType);

        if (!configs || configs.length === 0) {
            throw new ApiError('Không tìm thấy cấu hình giá cho sân này. Vui lòng liên hệ Admin.', 400);
        }

        const { totalPrice: basePrice, totalCalculatedMinutes } = this.calculateFromConfigs(configs, startDateTime, endDateTime);
        ...
        // 3. Lấy cấu hình hệ thống qua SystemConfigService (đã có cache)
